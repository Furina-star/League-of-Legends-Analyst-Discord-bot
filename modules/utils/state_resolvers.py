...
async def resolve_link_state(db, user_id: int, new_puuid: str, new_riot_id: str) -> tuple[bool, str]:
    current_link = await db.get_linked_account(user_id)
    ...
    if old_puuid != new_puuid:
        # self.db.clear_user_logs(user_id)
